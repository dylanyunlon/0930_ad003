...
//	bool bten = storage::get_bool(BT_POWER_KEY, true);
	LOGD("--%d-- --%s-- power on!   bten = %d \n", __LINE__, __FILE__, sys::setting::get_bt_power_key());
	if (!sys::setting::get_bt_power_key()) {				//蓝牙默认打开   所以主动恢复上次关闭状态
...
//	LOCK_OP(_s_info_mutex, _s_bt_info.call_start_time = LLONG_MAX);
	LOCK_OP(_s_info_mutex, _s_bt_info.talk_start_time = 0);
...
//	bool bten = storage::get_bool(BT_POWER_KEY, true);
	...
	if (sys::setting::get_bt_power_key()) {
		power_on();
	}
	if (btcn) {
...
std::string get_call_num() {
	...
	for (size_t i=0; i<_s_bt_info.contact_list.size(); i++) {
		if (_s_bt_info.contact_list[i].num == _s_bt_info.call_num) {
			return _s_bt_info.contact_list[i].num;
		}
	}
...
bool query_music_info() {
	return _send_cmd(BT_CMD_MUSIC_INFO);
}
