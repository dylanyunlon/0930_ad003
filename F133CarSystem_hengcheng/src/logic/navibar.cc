...
#define TIMER_SETBRIGHTNESS 1003
...
void set_navibar_phoneSeekBar(int progress) {
	if (Activity_is_load)
...
static void sys_time_(tm t ) {
	...
	sprintf(buf, "%02d-%02d", t.tm_mon+1, t.tm_mday, hour, t.tm_min);
...
public:
	virtual void onProgressChanged(ZKSeekBar *pSeekBar, int progress) {
		if (pSeekBar->getID() == mPlayVolSeekBarPtr->getID()) {
			...
			mphoneTextViewPtr->setText(progress*10);
...
//	mTextView1Ptr->setTouchPass(true);
	...
	mphoneButtonPtr->setTouchPass(true);
	...
	mbtTextViewPtr->setLongMode(ZKTextView::E_LONG_MODE_SCROLL_CIRCULAR);
}

static void set_fm_volume() {
	audio::set_system_vol(1.6);
}
...
static void _audio_output_mode_cb(audio_player_mode_e new_mode, audio_player_mode_e old_mode) {
	...
	case E_AUDIO_PLAYER_MODE_FM:
		mPlayVolSeekBarPtr->setTouchable(true);
		uart::fm_switch(false);
		sys::setting::set_fm_switch(false);
		break;
	default:
		break;
	}
	...
	switch (new_mode) {
	case E_AUDIO_PLAYER_MODE_FM:
		mPlayVolSeekBarPtr->setTouchable(false);
		mPlayVolSeekBarPtr->setProgress(mPlayVolSeekBarPtr->getMax());
		mvoiceNumTextViewPtr->setText((int)(audio::get_system_vol() * 100));
		uart::fm_switch(true);
		sys::setting::set_fm_switch(true);
		DELAY(120);
...
static void onUI_init() {
	...
	mphoneSeekBarPtr->setSeekBarChangeListener(&seekListener);
