...
static bool is_back = false;					//是否返回视频界面
static bool is_video_show = false;				//返回视频界面是否显示视频窗口

// 新增：记录进入视频播放界面的方式
typedef enum {
    E_VIDEO_ENTER_FROM_APP = 0,      // 从主应用界面进入
    E_VIDEO_ENTER_FROM_LIST = 1      // 从视频列表进入
} video_enter_type_e;

static video_enter_type_e video_enter_type = E_VIDEO_ENTER_FROM_APP;
...
//		iconRotate.requestExit();
	...
	mActivityPtr->unregisterUserTimer(VIDEO_CTRL_VIEW);
...
//	mtitleTextViewPtr->setText(filestr.substr(filestr.rfind("/")+1));
	...
	if (!ret) {		// 解析失败
		...
		goto FAIL;
	} else if ((info.codec_format >= VIDEO_CODEC_FORMAT_DIVX3 && info.codec_format <= VIDEO_CODEC_FORMAT_DIVX5)) {
		LOGD("Does not support DIVX3-DIVX5 format(0x%x)!\n", info.codec_format);
		goto FAIL;
...
static void _event_mode_cb(event_mode_e mode) {
	if (E_EVENT_MODE_GOBACK == mode) {
		if (mvideoPlayWindowPtr->isWndShow()) {
			// 如果视频播放窗口正在显示，根据进入方式决定返回行为
			if (video_enter_type == E_VIDEO_ENTER_FROM_LIST) {
				// 从列表进入的，返回列表
				...
				mVideoCtrlWindowPtr->hideWnd();
				mActivityPtr->unregisterUserTimer(VIDEO_CTRL_VIEW);
				video_enter_type = E_VIDEO_ENTER_FROM_APP;  // 重置状态
...
static bool onvideoActivityTouchEvent(const MotionEvent &ev) {
    switch (ev.mActionStatus) {
		case MotionEvent::E_ACTION_DOWN://触摸按下
			...
			mActivityPtr->resetUserTimer(VIDEO_CTRL_VIEW, 3000);
			break;
